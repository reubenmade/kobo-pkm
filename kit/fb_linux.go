...
// FB drives the Kobo e-ink framebuffer directly. The app draws into a logical
// portrait RGBA canvas; Refresh converts to the panel's native pixel
// format/rotation and issues a SEND_UPDATE ioctl telling the driver which
// region to refresh and with which waveform. See FIELD-NOTES.md for how these
// ioctls were reverse-engineered.
...
const (
	...
	// _IOW('F', 0x2E, struct) — struct size is encoded in the request number.
	mxcfbSendUpdateV1 = 0x4044462E // 68-byte v1_ntx (older NXP)
	mxcfbSendUpdateV2 = 0x4048462E // 72-byte v2 (NXP Mark 7+)

	// MediaTek "hwtcon" driver (Libra Colour & Mark 13+): same 'F' 0x2E command
	// but a 36-byte hwtcon_update_data struct. mxcfb callers get a baffling
	// EINTR forever on this driver — the struct size is the whole difference.
...
func OpenFB(cfg Config) (*FB, error) {
	...
	// fb id string is the first 16 bytes of the fixed info.
	idBytes := (*[16]byte)(unsafe.Pointer(&finfo[0]))[:]
	fbID := string(idBytes)
	for i := 0; i < len(fbID); i++ {
		if fbID[i] == 0 {
			fbID = fbID[:i]
			break
		}
	...
	// Rotation: apps are portrait; if the native fb is landscape, rotate.
	f.rot = 0
	if f.fbW > f.fbH {
		f.rot = 3
	}
	...
	log.Printf("fb: id=%q hwtcon=%v native %dx%d bpp=%d stride=%d app_rot=%d logical=%dx%d smem=%d",
		fbID, f.isHwtcon, f.fbW, f.fbH, f.bpp, f.stride, f.rot, lw, lh, smemLen)
	return f, nil
}
...
func (f *FB) blit(r image.Rectangle) image.Rectangle {
	...
	for ly := r.Min.Y; ly < r.Max.Y; ly++ {
		...
		for lx := r.Min.X; lx < r.Max.X; lx++ {
			...
			case 32:
				// Libra Colour panel is RGBA (per FBInk detection), not BGRA.
				f.mem[off] = cr
				f.mem[off+1] = cg
				f.mem[off+2] = cb
				f.mem[off+3] = 0xff
...
// sendHwtcon issues the MTK 36-byte hwtcon_update_data:
...
// ioctl retries on EINTR (EPD updates spend ~100ms in the kernel and get
// interrupted routinely) but with a deadline, so an interrupt storm can never
// wedge the app.
