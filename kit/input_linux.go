...
// Touch/pen input from evdev. The Kobo stylus (MPP) arrives through the same
// elan touchscreen as fingers. The device is EVIOCGRAB'd so nothing else
// reacts. The two facts that cost the most to learn:
//   - contact is PRESSURE-gated, never BTN_TOUCH (the elan holds BTN_TOUCH and
//     the tracking ID for the whole pen-in-range period, hover included);
//   - the pen streams while hovering (p=0, distance≈15) — a free channel.
...
const (
	...
	btnToolPen      = 0x140
	btnToolRubber   = 0x141
	btnStylus       = 0x14b // pen tail (eraser) on this digitizer — key 331
	btnStylus2      = 0x14c // pen side button — key 332
	absX            = 0x00
	absY            = 0x01
	absPressure     = 0x18
	absDistance     = 0x19
...
func findTouchDevice(override string) (string, error) {
	...
	if best == "" {
		best = "/dev/input/event1"
		log.Printf("input: no obvious touch device, defaulting to %s", best)
	}
	return best, nil
}

// OpenTouch opens and grabs the touchscreen, auto-detecting axis swap.
func OpenTouch(cfg Config, w, h int) (*TouchReader, error) {
	...
	// If the digitizer's aspect orientation disagrees with the display's, swap
	// x/y — unless the config forces it.
	if !cfg.SwapSet {
...
// logEach logs the first few occurrences of each distinct code, so a chatty
// code (the hover pulse, key 236) can't drown out a rare one (eraser, side
// button). Keyed by code only: a continuously-varying axis must not log once
// per value.
...
// Run parses evdev packets into Touch events on out. Full MT protocol-B slot
// tracking; only the primary contact streams Down/Move/Up. Hover frames (pen
// in range, not pressing) go out as TouchHover.
		...
		for off := 0; off+16 <= n; off += 16 {
			...
			case evAbs:
				...
				case absMTTrackingID:
					...
					if val < 0 {
						// Contact gone: drop stale pressure, or the next hover
						// reads the old value and inks instantly.
...
// GrabDevice exclusively grabs an input device by name fragment and drains its
// events. Used for the accelerometer — rotation events would otherwise make
// Nickel (or anyone) repaint over the framebuffer.
...
// OpenKeysByName finds a key device by name fragment. grab takes it exclusively
// (only possible with Nickel dead); leave the power key ungrabbed so a long
// hard press can always power the device off.
...
// usbOnline reports whether any USB/AC supply is online. NOTE: on the Libra
// Colour PMIC the "online" bit reads 1 permanently — prefer isCharging().
func usbOnline() bool {
