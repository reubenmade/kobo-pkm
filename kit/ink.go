...
// Ink captures pen strokes, renders them, erases them, and rasterizes the
// inked region (e.g. to send handwriting to a server OCR/vision pass). Ported
// from riddle's ink.rs. Contact/pressure gating happens in the input layer;
// callers feed already-resolved points and a brush radius.
...
func NewInk() *Ink { return &Ink{BBox: EmptyBBox()} }
...
// PenPoint: pen touched down or moved while down, with brush radius resolved by
// the caller (typically from pressure). Returns the dirty box of what changed.
...
// ErasePoint: brush white over the page AND drop stored points it covers, so
// the stroke model stays true to the visible page.
...
// ToPNG rasterizes the inked region to a grayscale PNG. Crops to the ink bbox
// and box-downscales so the long side stays ≤ 800px (image pixels dominate
// vision-token / latency cost, and handwriting reads fine at that scale).
...
// DissolvePass is one pass of a "drink the ink" fade: erase the pixels whose
// hash falls in this stage. After `stages` passes the region is clean white.
