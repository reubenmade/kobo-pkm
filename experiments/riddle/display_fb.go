...
// FB drives the Kobo's e-ink framebuffer directly — carried over from
// kobo-pkm/app (see FIELD-NOTES.md there for how these ioctls were found).
// The app draws into a logical portrait RGBA canvas; Refresh converts to the
// panel's native pixel format/rotation, then a SEND_UPDATE ioctl tells the
// display driver which region to refresh and with which waveform.
